...
export async function POST(_req: NextRequest, ctx: any) {
  ...
  if (source.userId === userId) return NextResponse.json({ error: "Cannot save your own roadmap" }, { status: 400 });

  // Idempotency: if the user already forked this source, return that roadmap instead of creating a duplicate
  const existing = await (prisma as any).roadmap.findFirst({ where: { userId, sourceId: source.id }, include: { progress: true } });
  if (existing) {
  ...
  const existing2 = await (prisma as any).roadmap.findFirst({ where: { userId, sourceId: source.id }, include: { progress: true } });
